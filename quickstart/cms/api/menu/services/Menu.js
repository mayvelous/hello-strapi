...
module.exports = {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Menu._attributes)
      .filter(attribute => attribute !== Menu.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Menu._attributes[attribute].type));
