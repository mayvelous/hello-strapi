...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Author.query(function(qb) {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Author._attributes)
      .filter(attribute => attribute !== Author.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Author._attributes[attribute].type));
